...
network.load_state_dict(torch.load('model_Fc.pkl'))
...
class MainWindow(QMainWindow,Ui_MainWindow):
    ...
    def pbtPredict_Callback(self):
        ...
        '''需要加载自己的训练网络，其他的代码不用更改'''
        network.eval() # 
        img_array = img_array.reshape(-1, 784)
        img_array =torch.from_numpy(img_array)
        img_array = img_array.to(device)
        img_array = img_array.to(torch.float32)
        __result = network(img_array)   # 正向传播得到预测值

        # 预测结果      
        self.result[0] = torch.argmax(__result, dim=1)   # 预测的数字
        self.result[1] = __result[0, self.result[0]]     # 置信度
        # 下面了两句可不用
        self.result[1] = self.result[1] * 2.0 
        if self.result[1] >= 1: self.result[1] = 1.0
